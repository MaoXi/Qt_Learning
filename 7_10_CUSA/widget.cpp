...
public:
        ...
        const QSize &size, const QWidget *widget) const
    {
        ...
        if (type == QStyle::CT_TabBarTab) {
            s.transpose();
            s.rwidth() = 110; // 璁剧疆姣忎釜tabBar涓璱tem鐨勫ぇ灏?
            s.rheight() = 80;
        }
        return s;
    }
        ...
        if (element == CE_TabBarTabLabel) {
            if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab *>(option)) {
                ...
                if (tab->state & QStyle::State_Selected) {
                    painter->save();
                    painter->setPen(0x000080);          //BLUE
                    painter->setBrush(QBrush(0x000080));
...
void Widget::on_spinBox_editingFinished()                 //参数量填写框
{
    ...
    subtraction = new double*[a];
    average = new double[a];
    rsd = new double[a];
    for(int i =0;i<a;i++){
        h[i] = new double[a];
        subtraction[i]= new double[a];
        average[i]=0;
        rsd[i]=0;
    }
    for(int i =0;i<a;i++){
...
//    qDebug()<<340;
    ...
    for(int i =0;i<global;i++){
        for(int j =0;j<global;j++){
            subtraction[i][j] = h[i][j] - s_Result[i][j];
        }
    }
//    qDebug()<<342;
}
...
void Widget::on_comboBox_6_activated(int index)
{
    ...
    //        qDebug()<<1;
           ...
           //设置坐标轴显示范围,否则我们只能看到默认的范围
            ui->qCustomPlot->xAxis->setRange(-5,5);
    ...
    switch(index)
   {
    ...
    case 0:
      {
        ...
        container->setMaximumSize(551,441);
    ...
    case 1:
       ...
       QWidget *container1 = QWidget::createWindowContainer(bar1);
        container1->setMaximumSize(551,441);
       // QGridLayout *mainLayout1= new QGridLayout;
    ...
    case 2:
       ...
       QWidget *container2 = QWidget::createWindowContainer(bar2);
        container2->setMaximumSize(551,441);
       // QGridLayout *mainLayout2= new QGridLayout;
       ...
       for(int m=(list.size())-1;m>=0;m--)
       {
           ...
           for(int n=0;n<list.size();n++)
           {
               (*dataRow2)[n].setValue(subtraction[n][m]);
              // hh--;
...
//    int i=1;
    //最末尾参数可以控制是否弹出控制台窗口SW_SHOW为显示，调用失败无提示，
    HINSTANCE hNewExe = ShellExecuteA(NULL, "open", "C:\\Users\\41688\\Desktop\\7_10_tab8_20\\cusa.exe",file_name, NULL, SW_SHOW);
    //不弹出控制台窗口
//    CreateProcess(TEXT("C:\\Users\\41688\\Desktop\\7_10_tab8_20\\cusa.exe"));
...
void Widget::on_pushButton_17_clicked()                            //显示文件列表，测试用
{
...
void Widget::on_pushButton_15_clicked()                                 //文件列表待选区---->选定区
{
...
void Widget::on_pushButton_18_clicked()                                //选定区--->待选区
{
...
void Widget::on_pushButton_11_clicked()                               //保存累积概率密度曲线图
{
    QPixmap pix;
    pix=ui->tabWidget->grab(QRect(440,160,400,330));
    QString fpath=QDateTime::currentDateTime().toString("yyyy-MM-dd HH-mm-ss")  + ".bmp";
    pix.save(QCoreApplication::applicationDirPath()+"//"+fpath,"bmp");
    QMessageBox::information(this,"tip","ok",QMessageBox::Ok);
}

void Widget::on_pushButton_16_clicked()                              //保存矩阵图
{
   /*
    QPixmap pix1;
    pix1=ui->groupBox_3->grab(QRect(0,0,700,601));
    QString fpath1=QDateTime::currentDateTime().toString("yyyy-MM-dd HH-mm-ss")  + ".bmp";
    pix1.save(QCoreApplication::applicationDirPath()+"//"+fpath1,"bmp");
   */
    QString fpath1=QDateTime::currentDateTime().toString("yyyy-MM-dd HH-mm-ss")  + ".bmp";
    QScreen *screen=QGuiApplication::primaryScreen();

    screen->grabWindow(this->winId(),130,100,561,400).save(QCoreApplication::applicationDirPath()+"//"+fpath1,"jpg");
